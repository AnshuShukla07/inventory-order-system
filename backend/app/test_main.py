...
def test_order_creation_success_and_stock_reduction():
    # 1. Create a customer
    res_cust = client.post("/api/customers/", json={"name": "Alice", "email": "alice@example.com"})
    cust_id = res_cust.json()["id"]

    # 2. Create products with stock
    res_p1 = client.post("/api/products/", json={"sku": "P1", "name": "Product 1", "price": 10.00, "stock_quantity": 100})
    res_p2 = client.post("/api/products/", json={"sku": "P2", "name": "Product 2", "price": 20.00, "stock_quantity": 50})
...
def test_order_creation_fails_insufficient_stock():
    ...
    res_p = client.post("/api/products/", json={"sku": "P1", "name": "Low Stock Product", "price": 10.00, "stock_quantity": 5})
    p_id = res_p.json()["id"]
...
def test_order_cancellation_restores_stock():
    ...
    res_cust = client.post("/api/customers/", json={"name": "Alice", "email": "alice@example.com"})
    cust_id = res_cust.json()["id"]
    res_p = client.post("/api/products/", json={"sku": "P1", "name": "Prod", "price": 10.00, "stock_quantity": 10})
    p_id = res_p.json()["id"]

    # 2. Create order
    res_order = client.post("/api/orders/", json={"customer_id": cust_id, "items": [{"product_id": p_id, "quantity": 4}]})
    ...
    # 3. Cancel order
    res_cancel = client.put(f"/api/orders/{order_id}/status", json={"status": "Cancelled"})
